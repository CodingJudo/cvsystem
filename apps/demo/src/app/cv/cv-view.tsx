...
import { RenderSpecEditor } from './render-spec-editor';
...
function CVContent({ warnings, initialCv }: { warnings: string[]; initialCv: DomainCV }) {
  ...
  const handleLoadFromFixtures = async () => {
    ...
    try {
      const result = await loadCvFromFixturesAction();
      if (result.success) {
        importCv(result.cv);
      ...
      <header className="sticky top-0 z-10 bg-[var(--brand-secondary)] text-white shadow-lg">
        <div className="mx-auto max-w-4xl px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Export Actions */}
            <Button size="sm" asChild className="bg-[var(--brand-accent)] hover:bg-[var(--brand-accent)]/90">
