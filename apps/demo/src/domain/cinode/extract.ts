...
import { DomainCVSchema } from '../model/cv.schema';
...
function extractSkills(raw: unknown, warnings: string[]): Skill[] {
  ...
  const addSkill = (skill: unknown) => {
    ...
    // Deduplicate by ID or exact name (trim, case-sensitive) so "React" and "react" stay separate
    if (id && seenIds.has(id)) return;
    if (seenNames.has(trimmedName)) return;
...
export function extractCv(rawSv: unknown, rawEn: unknown): ExtractionResult {
  ...
  // Validate our own output. A failure here is a programmer error in the extraction
  // logic — it should never happen in production and must be caught in tests.
  DomainCVSchema.parse(cv);
