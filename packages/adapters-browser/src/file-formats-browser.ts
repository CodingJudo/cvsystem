...
import { IMPORT_HISTORY_KEY, importFromFileInfos, importFromJson, serializeToJson, generateFilename } from '@cvsystem/core';
...
export async function importFiles(files: File[], options?: ImportOptions): Promise<ImportResult> {
  ...
  try {
    ...
    const result = importFromFileInfos(fileInfos, options);
...
export async function loadDemoCV(options?: ImportOptions): Promise<ImportResult> {
  try {
    ...
    const result = importFromJson(svData, enData, options);
