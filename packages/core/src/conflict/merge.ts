...
export function mergeWithResolutions(
  ...
  const hasAnyHobbyProjects = (cv: DomainCV): boolean => {
    return Boolean(cv.hobbyProjects && cv.hobbyProjects.length > 0);
  };
  const hasAnyTrainings = (cv: DomainCV): boolean => {
    return Boolean(cv.trainings && cv.trainings.length > 0);
  };
  const hasAnyEducations = (cv: DomainCV): boolean => {
    return Boolean(cv.educations && cv.educations.length > 0);
  };
  const hasAnyCommitments = (cv: DomainCV): boolean => {
    return Boolean(cv.commitments && cv.commitments.length > 0);
  };
  ...
  if (!hasAnyHobbyProjects(current) && hasAnyHobbyProjects(incoming)) {
    merged.hobbyProjects = incoming.hobbyProjects
      ? incoming.hobbyProjects.map((p) => ({
          ...p,
          description: { ...p.description },
          skills: p.skills.map((s) => ({ ...s })),
          skillOrder: p.skillOrder ? [...p.skillOrder] : undefined,
        }))
      : incoming.hobbyProjects;
  }
  // Trainings, educations, and commitments are never tracked as conflicts — always
  // prefer incoming when current is empty so they aren't silently lost during merge.
  if (!hasAnyTrainings(current) && hasAnyTrainings(incoming)) {
    merged.trainings = incoming.trainings.map((t) => ({ ...t }));
  }
  if (!hasAnyEducations(current) && hasAnyEducations(incoming)) {
    merged.educations = incoming.educations.map((e) => ({ ...e }));
  }
  if (!hasAnyCommitments(current) && hasAnyCommitments(incoming)) {
    merged.commitments = incoming.commitments.map((c) => ({ ...c }));
  }
