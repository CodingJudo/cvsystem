...
function ResolutionButtons({
  ...
  const t = {
    keepCurrent:
      currentLabel ?? (locale === "sv" ? "Behåll nuvarande" : "Keep current"),
    acceptIncoming:
      incomingLabel ??
      (locale === "sv" ? "Använd inkommande" : "Accept incoming"),
    ...
    return (
      <div className="flex gap-2 mt-3">
        ...
        <Button
          ...
          className={
            resolution === "accept" ? "bg-green-600 hover:bg-green-700" : ""
          }
        >
          {t.add}
        </Button>
    ...
    return (
      <div className="flex gap-2 mt-3">
        <Button
          ...
          className={resolution === "keep" ? "bg-brand-primary" : ""}
        >
          {t.keep}
        </Button>
        <Button
          ...
          onClick={() => onResolve("accept")}
          className={
            resolution === "accept" ? "bg-red-600 hover:bg-red-700" : ""
          }
        >
          {t.remove}
        </Button>
...
export function TextConflictCard({
  ...
  const currentText = getBilingualText(conflict.current, locale);
  const incomingText = getBilingualText(conflict.incoming, locale);

  return (
    <Card
      className={`border-l-4 ${resolution ? "border-l-green-500" : "border-l-amber-500"}`}
    >
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="p-3 bg-gray-50 rounded-lg">
            ...
            <p
              className={`text-sm ${!currentText ? "text-gray-400 italic" : ""}`}
            >
          ...
          <div className="p-3 bg-blue-50 rounded-lg">
            ...
            <p
              className={`text-sm ${!incomingText ? "text-gray-400 italic" : ""}`}
            >
...
export function RoleConflictCard({
  ...
  const title =
    role?.title ?? (locale === "sv" ? "Okänd roll" : "Unknown role");
  ...
  return (
    ...
    >
      <CardContent className="p-4">
        ...
        <div className="mb-2">
          <p className="font-medium text-brand-secondary">{title}</p>
              ...
              <div className="grid grid-cols-2 gap-4 mt-2">
                ...
                <div className="p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-xs text-gray-700 line-clamp-4">
                    {getBilingualText(
                      conflict.current?.description ?? { sv: null, en: null },
                      locale,
                    ) || "—"}
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    {conflict.current?.skills.length ?? 0}{" "}
                    {locale === "sv" ? "teknologier" : "technologies"}
                  </p>
                </div>
                ...
                <div className="p-3 bg-blue-50 rounded-lg">
                  <p className="text-xs font-medium text-blue-600 mb-1">
                    {locale === "sv"
                      ? "Inkommande beskrivning"
                      : "Incoming description"}
                  </p>
                  <p className="text-xs text-gray-700 line-clamp-4">
                    {getBilingualText(
                      conflict.incoming?.description ?? { sv: null, en: null },
                      locale,
                    ) || "—"}
                  </p>
                  <p className="text-xs text-blue-600 mt-1">
                    {conflict.incoming?.skills.length ?? 0}{" "}
                    {locale === "sv" ? "teknologier" : "technologies"}
                  </p>
                </div>
              </div>
