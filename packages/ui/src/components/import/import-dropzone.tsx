...
import type { ImportResult, FileValidationError } from '@cvsystem/core';
import { importFiles, validateFiles, loadDemoCV } from '../../lib/file-formats/browser';
...
export function ImportDropzone({ onImport, onCancel, disabled, locale = 'en' }: ImportDropzoneProps) {
  ...
  const t = {
    ...
    supported: locale === 'sv' ? 'Stöds: .json (Cinode eller CV System-format)' : 'Supported: .json (Cinode or CV System format)',
  ...
  return (
    <div className="space-y-4">
      {/* Loading overlay */}
      {isLoading && (
        <div className="flex items-center justify-center gap-3 p-4 bg-[var(--brand-primary)]/10 rounded-lg">
          <div className="w-5 h-5 border-2 border-[var(--brand-primary)] border-t-transparent rounded-full animate-spin" />
          <span className="text-[var(--brand-primary)] font-medium">{loadingMessage}</span>
        </div>
      )}
      ...
      {!isLoading && (
        <div
          ...
          className={`
            ...
            ${isDragging 
              ...
              : 'border-gray-300 hover:border-[var(--brand-primary)]/50 hover:bg-gray-50'
            }
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <p className="text-lg font-medium text-[var(--brand-secondary)]">
              {t.dropFiles}
            </p>
      ...
      <div className="flex gap-3 justify-between pt-2">
        <Button
          ...
          className="text-[var(--brand-primary)] border-[var(--brand-primary)] hover:bg-[var(--brand-primary)]/10"
        >
          {t.loadDemo}
        </Button>
