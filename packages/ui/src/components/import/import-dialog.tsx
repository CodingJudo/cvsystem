...
import { detectConflicts, type ConflictAnalysis } from '@cvsystem/core';
...
export function ImportDialog({ 
  ...
  // A CV is considered "blank" when it has no roles and no skills — i.e. the user just cleared it.
  // In that case there is nothing to conflict-resolve; skip straight to import.
  const currentHasContent = Boolean(
    currentCv && (currentCv.roles.length > 0 || currentCv.skills.length > 0)
  );

  const handleImport = (result: ImportResult) => {
    setImportResult(result);

    if (result.success && hasExistingData && currentHasContent && currentCv && result.cv) {
