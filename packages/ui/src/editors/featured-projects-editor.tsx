...
import type { FeaturedProject, Locale } from "@cvsystem/core";
import { useCVActions, useCVState, useRoles } from "../store/cv-store";
...
export function FeaturedProjectsEditor({ locale }: { locale: Locale }) {
  ...
  const { addFeaturedProject, updateFeaturedProject, deleteFeaturedProject } =
    useCVActions();
  ...
  const availableRoles = useMemo(() => {
    const used = new Set(
      projects.map((p) => p.roleId).filter(Boolean) as string[],
    );
              ...
              <FeaturedProjectCard
                ...
                onChangeCompany={(value) => {
                  ...
                  updateFeaturedProject(p.id, {
                    company: value.trim() ? value : null,
                  });
                }}
                onChangeRoleTitle={(value) => {
                  if (!cv) return;
                  updateFeaturedProject(p.id, {
                    roleTitle: value.trim() ? value : null,
                  });
                }}
                ...
                onToggleVisible={() =>
                  updateFeaturedProject(p.id, { visible: !p.visible })
                }
...
function FeaturedProjectCard(props: {
  ...
  return (
    <div
      className={`rounded-lg border p-4 ${project.visible ? "border-gray-200" : "border-gray-200 opacity-50"}`}
    >
          ...
          {isEditing ? (
            <div className="grid gap-2">
              <Input
                ...
                placeholder={
                  locale === "sv" ? "Företag/uppdragsgivare" : "Company/client"
                }
            ...
            <>
              <div className="font-semibold text-[var(--brand-secondary)]">
                {project.company ?? "—"}
              </div>
            ...
            <Textarea
              ...
              placeholder={
                locale === "sv"
                  ...
                  : "Short project highlight…"
              }
            />
          </div>
        ) : description ? (
          <div className="text-sm text-gray-700 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:my-1 [&_li]:my-0.5">
            <BasicMarkdownText text={description} className="m-0" />
          </div>
        ) : (
          <p className="text-sm text-gray-400 italic">
            {locale === "sv"
              ? "Ingen highlight-text ännu."
              : "No highlight text yet."}
          </p>
        )}
      </div>
    </div>
